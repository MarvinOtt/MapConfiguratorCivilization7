...
namespace MapConfiguratorCivilization7
{
    ...
    public class ImGuiRenderer
    {
        ...
        public virtual IntPtr BindTextureOverrideID(Texture2D texture, IntPtr ID)
        {
        ...
        /// </summary>
        public virtual void BeforeLayout(GameTime gameTime, bool IsIOUpdate = true)
        {
            if (IsIOUpdate)
            ...
            if (IsIOUpdate)
                UpdateInput();
            var io = ImGui.GetIO();
            io.DisplaySize.X = _graphicsDevice.Viewport.Width;
            io.DisplaySize.Y = _graphicsDevice.Viewport.Height;
            ImGui.NewFrame();
        }
        ...
        public unsafe void SetupCustomFont(string filename, float size)
        {
            ImGui.GetIO().Fonts.AddFontFromFileTTF(filename, size);
        }

        /// <summary>
        /// Maps ImGui keys to XNA keys. We use this later on to tell ImGui what keys were pressed
        /// </summary>
        protected unsafe virtual void SetupInput()
        {
            var io = ImGui.GetIO();
